...
NOTE: 为了测试或模拟一个或多个驱动器故障，请使用 `storage disk fail -disk NET-x.y -immediate`命令。如果系统中有备用磁盘，聚合将开始重建。您可以使用以下命令检查重建的状态 `storage aggregate show`。您可以使用ONTAP Deploy 移除模拟故障驱动器。请注意，ONTAP已将硬盘标记为 `Broken`。驱动器实际上并未损坏，可以使用ONTAP Deploy 重新添加。要清除“Broken”标签，请在ONTAP Select CLI 中输入以下命令
