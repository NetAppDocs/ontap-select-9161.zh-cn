...
有关详细信息，请参阅link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["双节点扩展 HA (MetroCluster SDS) 最佳实践"] 。
...
NOTE: ONTAP保留端口 e0e 用于内部复制流量。因此，该端口和托管 LIF 在ONTAP命令行界面 (CLI) 或系统管理器 (System Manager) 中均不可见。此接口配置为使用自动生成的链路本地 IP 地址，不支持重新分配备用 IP 地址。此网络端口需要使用巨型帧（7500 到 9000 MTU）。
