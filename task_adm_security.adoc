...
| 密码 | 输入关联用户名的密码。 
|===
. 添加新的管理服务器后，您可以选择单击image:icon_kebab.gif["选项"]并选择以下选项之一：
+
** 更新凭据
** 验证凭证
** 删除管理服务器
...
配置 YubiKey PIN 并按照以下步骤生成或导入远程支持代理 (RSA) 或椭圆曲线数字签名算法 (ECDSA) 私钥和证书link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647： ONTAP中的多因素身份验证"^] 。
...
如果您选择使用 YubiKey FIDO2 身份验证，请使用 YubiKey 管理器配置 YubiKey FIDO2 PIN 码，并使用 PuTTY-CAC（通用访问卡） (Windows) 或 ssh-keygen (MacOS) 生成 FIDO2 密钥。具体步骤请参阅技术报告。 link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647： ONTAP中的多因素身份验证"^] 。
