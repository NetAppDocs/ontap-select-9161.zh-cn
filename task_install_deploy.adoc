...
.关于此任务
ONTAP Select Deploy 管理实用程序基于开放虚拟化格式 (OVF) 标准打包为虚拟机 (VM)。单个压缩文件的后缀为 `ova` 。虚拟机为ONTAP Select节点提供 Deploy 服务器和安装映像。
...
| `ONTAPdeploy.ova.sig` | SHA-256 算法经过哈希处理，然后由远程支持代理 (RSA) 使用 `csc-prod`安装程序的密钥和签名。 
|===
. 验证 `ONTAPdeploy.ova.sig`文件正在使用相关的证书和验证命令。
. 使用以下命令验证签名：
+
[listing]
----
