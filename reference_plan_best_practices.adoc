...
NOTE: 在 ESX 6.5 U2 及更高版本上安装时， ONTAP Select都会使用 vNVME 驱动程序，无论系统磁盘位于 SSD 还是 NVME 磁盘上。这会将虚拟机硬件级别设置为 13，与 ESX 6.5 及更高版本兼容。
...
要了解如何在虚拟机管理程序主机上配置 Open vSwitch，请参阅link:https://www.netapp.com/media/13134-tr4613.pdf["ONTAP Select on KVM 产品架构和最佳实践"^]技术报告。
...
最佳做法是定期备份 Deploy 配置数据，包括在创建集群后。这对于双节点集群尤为重要，因为备份中包含了中介器配置数据。

创建或部署集群后，您应该备份ONTAP Select Deploy 配置数据。
